...
export numfiles=`cat atac.tagalign.ppr.txt| wc -l`
echo $numfiles
for i in $(seq 1 $numfiles)
do
    cur_sample_name=`head -n $i atac.tagalign.ppr.txt | tail -n1 | cut -f1`
    echo $cur_sample_name > counts.$cur_sample_name.txt
    cur_tagalign=`head -n $i atac.tagalign.ppr.txt | tail -n1 | cut -f2`
...
paste counts.*.txt > hydrogel.atac.counts.ppr.txt
#clean up temporary files
rm counts.*.txt
paste naive_overlap.optimal_set.sorted.merged.bed hydrogel.atac.counts.ppr.txt > tmp
mv tmp hydrogel.atac.counts.ppr.txt
